...
def main(filter_cut: float, walk_min: float, walk_max: float, npileup: tuple = None)->None:
    ...
    if npileup is not None:
        gen_particles = gen_particles[(npileup[0]<= gen_particles['npileup']) &(gen_particles['npileup'] <= npileup[1])]
        reco_particles = reco_particles[(npileup[0]<= reco_particles['npileup']) &(reco_particles['npileup'] <= npileup[1])]
        match_particles = match_particles[(npileup[0]<= match_particles['npileup']) &(match_particles['npileup'] <= npileup[1])]
    # Filter out particles with no hits
    data = {
        'gen': len(gen_particles),
        'reco': len(reco_particles),
        'matched': len(match_particles),
        'eff': len(match_particles)/len(reco_particles),
        'ratio': 1 - len(reco_particles)/len(reco_particles),
...
def fakeAndMisreconstructed_2d(error, index, convert_index):
    ...
    plt.annotate(rf'at ({convert_index[min_idx[1]]}, {convert_index[min_idx[0]]})',
            ...
            xytext=(min_idx[1] - 5, min_idx[0]),  # text position
            #arrowprops=dict(facecolor='red', shrink=0.05),
            )
...
def efficiency_2d(efficiency, index, convert_index, name):
    ...
    plt.annotate(rf'Best efficiency: {max_value:.3f} ($\pm$ {eff_error[index][max_idx]:.3f})',
            xy=(max_idx[1], max_idx[0]),  # point to annotate
            xytext=(0, -30),  # text position
            color = 'red',
            ha = 'center',
...
if __name__ == "__main__":
    ...
    for raw in read_parameter_file('experiment/grid_list.txt'):
        results = main(raw['filter_cut'], raw['walk_min'], raw['walk_max'], npileup[npileup_bin])
        if results is None: continue
        eff, error, e_rate = results['eff'], results['eff-error'], results['error']
    ...
    for index, cut in convert_index.items():
        ...
        efficiency_2d(efficiency, index, convert_index, name = 'Efficiency')