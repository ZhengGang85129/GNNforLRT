...
def prepare_pipeline() -> None:
    ...
    for Idx, (condition, stage) in enumerate(zip(pipeline['stage_list'], stages)):
        if stage in args.only:
            condition['config'] = f'train-{args.sample}-{args.task_name}.yaml'
...
def write_script()->None:
    ...
    with open('tmp.sh', 'w') as f:
        ...
        f.write('#SBATCH --time 5:0:0\n') 
        # Load any required modules

        f.write('export SLURM_CPU_BIND="cores"\n')

# Change to your working directory
        f.write('cd /global/homes/z/zhenggan/workspace/GNNforLRT/pipelines/pileup_40_new\n')
        f.write('pwd\n')
        f.write('bash\n')
        f.write('nvidia-smi\n')
        f.write('which bash\n')
        ...
        if "GNN" in args.only:
            with open(f'./LightningModules/GNN/train-{args.sample}-{args.task_name}.yaml', 'r') as f0:
